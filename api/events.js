...
export default async function handler(req, res) {
  // CORS — needed for Scriptable widget requests
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'GET, POST, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type');
  if (req.method === 'OPTIONS') return res.status(200).end();

  // Check KV is connected
  if (!process.env.KV_REST_API_URL) {
    return res.status(503).json({
      error: 'Vercel KV not connected. Go to your Vercel project → Storage → Create KV Database → Connect to project → Redeploy.',
    });
  }
  ...
  try {
    ...
    return res.status(500).json({ error: 'Storage error — check Vercel KV is connected' });
  }
}
