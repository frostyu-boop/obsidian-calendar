...
const expandEvents = (events, from, to) => {
  const fromS = fmtDate(from), toS = fmtDate(to);
  const result = [];
...
const loadEvts = () => {
  try { const d = localStorage.getItem(SK); return d ? JSON.parse(d) : []; }
  catch { return []; }
};
const saveEvts = evts => {
  try { localStorage.setItem(SK, JSON.stringify(evts)); } catch {}
};
// Shared data for the future Workout app
const saveWorkoutData = events => {
  try {
    const gym = events.filter(e => e.category === "Gym/Sport");
...
const fireNotif = evt => {
  if ("Notification" in window && Notification.permission === "granted")
    new Notification(`📅 ${evt.title}`, { body: evt.allDay ? "All day event" : `At ${fmtTime(evt.startTime)}` });
};
    ...
    <div style={{ padding:"0 10px" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:"1px" }}>
        {cells.map((day,i)=>{
          ...
          return (
            <div key={i} className="day-cell" onClick={()=>onDayClick(d)} style={{
              aspectRatio:"1", display:"flex", flexDirection:"column", alignItems:"center",
              padding:"5px 2px 3px", borderRadius:11, cursor:"pointer", transition:"background 0.13s",
              background:tod?"#7C6AF7":"transparent",
            }}>
              ...
              <div style={{ display:"flex", gap:2, marginTop:3 }}>
                {evts.slice(0,3).map((e,ei)=>(
                  <div key={ei} style={{ width:4,height:4,borderRadius:"50%",background:CATS[e.category]?.color||"#555" }}/>
                ))}
              </div>
            </div>
          );
        ...
        {days.map((d,di)=>{
          ...
          return (
            <div key={di} style={{ flex:1,padding:"4px 1px" }}>
              ...
              {ade.map((e,ei)=>(
                <div key={ei} onClick={()=>onEventClick(e)} style={{ fontSize:8,padding:"1px 3px",borderRadius:3,marginTop:1,background:CATS[e.category]?.color+"30",color:CATS[e.category]?.color,overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis",cursor:"pointer",fontWeight:700 }}>{e.title}</div>
              ))}
            </div>
          );
        })}
                ...
                {de.map(evt=>{
                  ...
                  return (
                    <div key={evt.id} onClick={e=>{e.stopPropagation();onEventClick(evt);}} style={{
                      ...
                      background:CATS[evt.category]?.dim,borderLeft:`2px solid ${CATS[evt.category]?.color}`,
                      borderRadius:"0 4px 4px 0",padding:"2px 4px",overflow:"hidden",cursor:"pointer",zIndex:1,
                    }}>
  ...
  return (
    <div className="fade" style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.78)",zIndex:300,display:"flex",alignItems:"flex-end" }}
      ...
      <div className="sheet" style={{ width:"100%",maxHeight:"93vh",background:"#0F0F0F",borderRadius:"22px 22px 0 0",overflowY:"auto",padding:"14px 18px",paddingBottom:"max(48px, env(safe-area-inset-bottom, 48px))" }}>
        ...
        <input value={f.title} onChange={e=>set("title",e.target.value)} placeholder="Event title" autoFocus
          style={{ ...S.input,fontSize:17,fontWeight:500,marginBottom:12 }}/>

        <div style={{ marginBottom:12 }}>
          <div style={S.label}>Category</div>
          <div style={{ display:"flex",flexWrap:"wrap",gap:7 }}>
            {Object.entries(CATS).map(([c,{color}])=>(
              <button key={c} className="tap" onClick={()=>set("category",c)} style={{
                padding:"5px 13px",borderRadius:20,border:`1.5px solid ${color}`,
                background:f.category===c?color:"transparent",color:f.category===c?"#fff":color,
                fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"DM Sans,sans-serif",transition:"all 0.15s",
              }}>{c}</button>
            ))}
          </div>
        </div>
          ...
          {[["Start Date","startDate"],["End Date","endDate"]].map(([l,k])=>(
            <div key={k} style={{ background:"#181818",border:"1px solid #242424",borderRadius:12,padding:"10px 12px" }}>
              ...
              <input type="date" value={f[k]} onChange={e=>set(k,e.target.value)}
                style={{ background:"none",border:"none",color:"#DDD",fontSize:13,fontFamily:"DM Sans,sans-serif",width:"100%",outline:"none",cursor:"pointer" }}/>
            </div>
          ))}
        </div>
        ...
        <input value={f.location||""} onChange={e=>set("location",e.target.value)} placeholder="📍 Location"
          style={{ ...S.input,marginBottom:10,color:"#A0A0A0" }}/>
        ...
        <div style={{ marginBottom:10 }}>
          ...
          <div style={{ display:"flex",gap:7,flexWrap:"wrap" }}>
            {REPEAT_OPTS.map(({v,l})=>(
              <button key={v} className="tap" onClick={()=>set("repeat",v)} style={{
                padding:"6px 13px",borderRadius:20,
                border:`1.5px solid ${f.repeat===v?"#7C6AF7":"#282828"}`,
                background:f.repeat===v?"rgba(124,106,247,0.14)":"transparent",
                color:f.repeat===v?"#A898FF":"#555",
                fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"DM Sans,sans-serif",transition:"all 0.15s",
              }}>{l}</button>
            ))}
          </div>
        ...
        <div style={{ marginBottom:14 }}>
          ...
          <select value={f.notification??15} onChange={e=>set("notification",Number(e.target.value))}
        ...
        {event && (
          <div style={{ display:"flex",gap:8 }}>
            <button className="tap" onClick={()=>onSave({...f,id:genId(),title:`${f.title} (copy)`})} style={{
              flex:1,padding:"12px",background:"#181818",border:"1px solid #282828",borderRadius:12,
              color:"#888",fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:"DM Sans,sans-serif",
            }}>Duplicate</button>
            <button className="tap" onClick={()=>{onDelete(event._pid||event.id);onClose();}} style={{
              flex:1,padding:"12px",background:"rgba(255,60,60,0.06)",border:"1px solid rgba(255,60,60,0.22)",
              borderRadius:12,color:"#FF6060",fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:"DM Sans,sans-serif",
            }}>Delete</button>
          </div>
        )}
      </div>
...
function DayModal({ date, events, onClose, onEventClick, onAddNew }) {
  const expanded = useMemo(()=>
    expandEvents(events,date,date)
      .sort((a,b)=>(a.allDay?-1:b.allDay?1:0)||((a.startTime||"")>(b.startTime||"")?1:-1))
  ,[events,date]);
    ...
    <div className="fade" style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.78)",zIndex:300,display:"flex",alignItems:"flex-end" }}
      ...
      <div className="sheet" style={{ width:"100%",maxHeight:"74vh",background:"#0F0F0F",borderRadius:"22px 22px 0 0",overflowY:"auto",padding:"14px 18px",paddingBottom:"max(48px, env(safe-area-inset-bottom, 48px))" }}>
        ...
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18 }}>
          <div>
            <div style={{ fontFamily:"Syne,sans-serif",fontWeight:800,fontSize:22,color:"#F0F0F0" }}>
              {MONTHS[date.getMonth()]} {date.getDate()}
            </div>
            <div style={{ color:"#484848",fontSize:12,fontFamily:"DM Sans,sans-serif",marginTop:2 }}>
              {DOWS_FULL[date.getDay()]}
              {isToday(date)&&<span style={{ color:"#7C6AF7",marginLeft:8,fontWeight:700 }}>· Today</span>}
            </div>
          </div>
        ...
        {expanded.length===0 ? (
          <div style={{ textAlign:"center",padding:"28px 0",color:"#303030",fontSize:14,fontFamily:"DM Sans,sans-serif" }}>
            No events · tap + Add to create one
          </div>
        ) : expanded.map(evt=>(
          <div key={evt.id} onClick={()=>{onEventClick(evt);onClose();}} style={{
            display:"flex",alignItems:"center",gap:12,padding:"12px 14px",
            background:"#161616",border:"1px solid #1F1F1F",borderRadius:14,marginBottom:8,cursor:"pointer",
            borderLeft:`3px solid ${CATS[evt.category]?.color||CATS.Other.color}`,
          }}>
...
function UpcomingList({ events, onEventClick }) {
  ...
  const list = useMemo(()=>
    expandEvents(events,new Date(),end30)
      .filter(e=>e.endDate>=tod)
      .sort((a,b)=>a.startDate>b.startDate?1:a.startDate<b.startDate?-1:(a.startTime||"")>(b.startTime||"")?1:-1)
      .slice(0,8)
  ,[events]);

  if (!list.length) return (
    <div style={{ padding:"24px 14px",textAlign:"center",color:"#282828",fontSize:13,fontFamily:"DM Sans,sans-serif" }}>
      Nothing coming up · tap + to add events
    </div>
  );
  ...
  return (
    <div style={{ padding:"18px 12px 8px" }}>
      ...
      {list.map(evt=>(
        <div key={evt.id} onClick={()=>onEventClick(evt)} style={{
          display:"flex",alignItems:"center",gap:11,marginBottom:7,
          padding:"10px 12px",background:"#111",border:"1px solid #181818",borderRadius:13,cursor:"pointer",
        }}>
...
function LegendModal({ onClose }) {
  return (
    <div className="fade" style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.78)",zIndex:300,display:"flex",alignItems:"flex-end" }}
      onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="sheet" style={{ width:"100%",background:"#0F0F0F",borderRadius:"22px 22px 0 0",padding:"14px 20px",paddingBottom:"max(40px, env(safe-area-inset-bottom, 40px))" }}>
        <div style={{ width:34,height:4,borderRadius:2,background:"#282828",margin:"0 auto 14px" }}/>
        <div style={{ fontFamily:"Syne,sans-serif",fontWeight:800,fontSize:17,color:"#F0F0F0",marginBottom:18 }}>Categories</div>
        {Object.entries(CATS).map(([cat,{color,dim}])=>(
          <div key={cat} style={{ display:"flex",alignItems:"center",gap:14,marginBottom:14 }}>
            ...
            <span style={{ color:"#CECECE",fontSize:15,fontFamily:"DM Sans,sans-serif",fontWeight:500 }}>{cat}</span>
            <div style={{ flex:1,height:6,borderRadius:3,background:dim }}/>
          </div>
        ))}
      </div>
    </div>
...
export default function App() {
  ...
  // Load once on mount (synchronous)
  useEffect(()=>{
    setEvents(loadEvts());
    setLoaded(true);
    reqNotif();
  ...
  useEffect(()=>{
    if (loaded) { saveEvts(events); saveWorkoutData(events); }
  },[events, loaded]);

  // Notification poller (checks every minute while app is open)
  useEffect(()=>{
  ...
  return (
    <div style={{
      ...
      // iPhone safe area — top (Dynamic Island / notch)
      ...
      <div style={{ display:"flex",margin:"0 18px 12px",background:"#111",borderRadius:13,padding:3,border:"1px solid #1A1A1A",flexShrink:0 }}>
        {["month","week"].map(v=>(
          <button key={v} className="tap" onClick={()=>setView(v)} style={{
            ...
            background:view===v?"#7C6AF7":"transparent",color:view===v?"#fff":"#404040",
            fontSize:12,fontWeight:800,transition:"all 0.2s",fontFamily:"Syne,sans-serif",
      ...
      <div style={{
        ...
        borderTop:"1px solid #161616",flexShrink:0,
      }}>
        <button className="tap" onClick={()=>setDate(new Date())} style={{
          background:"none",border:"none",color:"#505050",fontSize:11,cursor:"pointer",
          fontFamily:"Syne,sans-serif",fontWeight:800,letterSpacing:0.8,textTransform:"uppercase",padding:"8px 10px",
        }}>Today</button>

        <button className="tap" onClick={()=>setEvtModal({defDate:todayStr()})} style={{
          width:52,height:52,borderRadius:"50%",
          background:"linear-gradient(135deg,#7C6AF7,#B57BF7)",border:"none",
          color:"#fff",fontSize:26,cursor:"pointer",
          display:"flex",alignItems:"center",justifyContent:"center",
          boxShadow:"0 4px 28px rgba(124,106,247,0.5)",
        }}>+</button>

        <button className="tap" onClick={()=>setShowLeg(true)} style={{
          background:"none",border:"none",cursor:"pointer",padding:"8px 10px",
          display:"flex",gap:3,alignItems:"center",
        }}>
          {Object.values(CATS).slice(0,4).map((c,i)=>(
            <div key={i} style={{ width:6,height:6,borderRadius:"50%",background:c.color }}/>
          ))}
        </button>
      </div>
      ...
      {/* Modals */}
      {dayModal && (
        <DayModal date={dayModal} events={events} onClose={()=>setDayModal(null)}
          onEventClick={e=>setEvtModal({event:e})}
          onAddNew={()=>{ setEvtModal({defDate:fmtDate(dayModal)}); setDayModal(null); }}
        />
      )}
      {evtModal && (
        <EventModal event={evtModal.event} defDate={evtModal.defDate} defHour={evtModal.defHour}
          onSave={save} onDelete={del} onClose={()=>setEvtModal(null)}/>
      )}
      {showLeg && <LegendModal onClose={()=>setShowLeg(false)}/>}
    </div>
  );
}
